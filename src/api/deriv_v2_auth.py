"""
Deriv Developers API (v2) authentication helpers for binance ai bot.
...
def list_options_accounts(
    ...
    *,
    ...
    headers = api_headers(app_id, bearer_token)
    ...
    try:
        ...
        logger.error("list_options_accounts HTTP error: %s", e)
        raise
    ...
    logger.warning("Unexpected accounts payload keys: %s", list(payload.keys()) if isinstance(payload, dict) else type(payload))
    return []
...
def pick_account(
    ...
    *,
    ...
    def _is_demo(a: Dict[str, Any]) -> bool:
        ...
        if aid.startswith(("VRT", "VRTC", "VRW", "DOT")):
            return True
        if "demo" in group:
            return True
        return False
...
def fetch_otp_websocket_url(
    ...
    *,
    ...
    url = f"{api_base.rstrip('/')}/trading/v1/options/accounts/{account_id}/otp"
    headers = api_headers(app_id, bearer_token)
    ...
    try:
        ...
        logger.error("fetch_otp HTTP error: %s", e)
        raise
...
def resolve_authenticated_ws_url(
    app_id: str,
    bearer_token: str,
    *,
    mode: str = "demo",
    ...
    """
    accounts = list_options_accounts(app_id, bearer_token, api_base=api_base)
    if not accounts:
        raise RuntimeError(
            "No options trading accounts returned from Deriv API v2."
        )
    ...
    ws_url = fetch_otp_websocket_url(
